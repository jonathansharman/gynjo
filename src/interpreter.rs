...
pub fn eval_expr(env: &Rc<Env>, expr: &Expr) -> EvalResult {
	match expr {
		Expr::Cond { test, then_expr, else_expr } => {
			eval_expr(&env, test).and_then(|test_value| {
				match test_value {
					Value::Literal(Literal::Boolean(boolean)) => eval_expr(&env, if boolean.into() { then_expr } else { else_expr }),
		...
		Expr::TupleExpr(expr_elems) => {
			let elems = Box::new(Vec::with_capacity(expr_elems.len()));
			for value in expr_elems.iter().map(|elem| eval_expr(&env, elem)) {
				elems.push(value?);
			}
		...
		Expr::ListExpr(expr_elems) => {
			...
			for expr_elem in expr_elems.iter() {
				list = List::Cons { head: Box::new(eval_expr(&env, expr_elem)?), tail: Rc::new(list) };
			}
			Ok(Value::List(list))
		},
...
pub fn eval(env: &Rc<Env>, input: &str) -> EvalResult {
	// Lex.
...
pub fn exec_stmt(env: &mut Rc<Env>, stmt: &Stmt) -> ExecResult {
	match stmt {
		Stmt::Nop => Ok(()),
		...
		Stmt::WhileLoop { test, body } => {
			loop {
				...
				let test_value = eval_expr(env, test).map_err(|err| format!("in while-loop test expression: {}", err))?;
				match test_value {
			...
			match range_value {
				Value::List(range_list) => {
					for value in range_list.iter() {
						...
						exec_stmt(env, body).map_err(|err| format!("in body of for-loop: {}", err))?;
					}
					Ok(())
				},
		...
		Stmt::ExprStmt(expr) => {
			...
			if let Value::Tuple(Tuple { elems }) = value {
				if elems.is_empty() {
					return Ok(());
				}
			}
			Err(format!("unused expression result: {}", value.to_string(env)))
		}
	}
}
...
pub fn exec(env: &mut Rc<Env>, input: &str) -> ExecResult {
	...
	while !token_slice.is_empty() {
		...
		// Execute.
		exec_stmt(&mut env, &stmt).map_err(|err| format!("(runtime error) {}", err))?;
	}
	Ok(())
}

/// Evaluates a numerical operation on a list and a number.
fn list_num_op<F>(env: &Rc<Env>, list: &List, number: &BigDecimal, number_on_left: bool, op_name: &str, op: F) -> Result<List, String> where F: (FnOnce(&BigDecimal, &BigDecimal) -> EvalResult) {
	match list {
		List::Empty => Ok(List::Empty),
		List::Cons { head, tail } => Ok(List::Cons {
			head: if number_on_left {
				Box::new(bin_num_op(env, &Value::Literal(Literal::Number(*number)), head, op_name, op)?)
			} else {
				Box::new(bin_num_op(env, head, &Value::Literal(Literal::Number(*number)), op_name, op)?)
			},
			tail: Rc::new(list_num_op(env, tail, number, number_on_left, op_name, op)?),
		}),
	}
}

/// Evaluates a numerical operation on two values.
fn bin_num_op<F>(env: &Rc<Env>, left: &Value, right: &Value, op_name: &str, op: F) -> EvalResult where F: (FnOnce(&BigDecimal, &BigDecimal) -> EvalResult) {
	match (left, right) {
		// Number op Number
...
fn eval_cluster(env: &Rc<Env>, cluster: &Cluster) -> EvalResult {
	// First, evaluate the cluster items.
	let evaluated_cluster: Vec<EvaluatedClusterItem> = Vec::with_capacity(cluster.items.len());
...
fn eval_application(c: &Closure, args: &Vec<Value>) -> EvalResult {
	...
	let local_env = Rc::new(Env::new(Some(c.env)));
	for (variable, value) in c.f.params.iter().zip(args.iter()) {
		local_env.assign(*variable, *value);
	}
	// Evaluate function body within the application environment.
	match c.f.body {
		LambdaBody::UserDefined(body) => eval_expr(&local_env, &*body),
		LambdaBody::Intrinsic(body) => {
			match body {
				Intrinsic::Top => match local_env.lookup(&"list".into()).unwrap() {
					Value::List(List::Cons { head, .. }) => Ok(**head),
				...
				Intrinsic::Pop => match local_env.lookup(&"list".into()).unwrap() {
					Value::List(List::Cons { tail, .. }) => Ok(Value::List(**tail)),
				...
				Intrinsic::Push => match local_env.lookup(&"list".into()).unwrap() {
					Value::List(list) => {
						let value = *local_env.lookup(&"value".into()).unwrap();
						Ok(Value::List(List::Cons { head: Box::new(value), tail: Rc::new(*list) }))
					},
